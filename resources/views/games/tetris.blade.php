...
<div class="container py-4">
    <div class="d-flex align-items-center justify-content-between mb-4 flex-wrap gap-3">
        ...
        <div class="d-flex gap-2 flex-wrap">
            <span class="info-badge" style="--bc:#00d2ff">PUZZLE</span>
            <span class="info-badge" style="--bc:#00d2ff">ARROW KEYS</span>
    ...
    <div class="row g-4">
        {{-- Canvas --}}
        <div class="col-lg-8">
            <div class="game-canvas-wrap" style="--gc:#00d2ff; background:#000d1a; border-color:rgba(0,210,255,0.3); box-shadow:0 0 40px rgba(0,210,255,0.1);">
                ...
                <div id="tetris-overlay" class="game-overlay" style="background:rgba(0,13,26,0.9);">
                    <div class="overlay-content">
                        ...
                        <h2 class="overlay-title" style="color:#00d2ff">CYBER_TETRIS</h2>
                        <p class="overlay-sub">Stack neon blocks and clear lines to score!<br>Speed increases with each level.</p>
                        <button id="tetris-start-btn" class="game-btn" style="--bc:#00d2ff">▶ START GAME</button>
                    </div>
                </div>
                <div id="tetris-gameover" class="game-overlay" style="display:none; background:rgba(0,13,26,0.9);">
                    <div class="overlay-content">
                        ...
                        <p class="overlay-sub">Score: <span id="tet-final-score" style="color:#00d2ff; font-family:'Orbitron',sans-serif; font-size:1.4rem; font-weight:700;">0</span></p>
                        <button id="tetris-restart-btn" class="game-btn" style="--bc:#00d2ff">⟳ PLAY AGAIN</button>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="col-lg-4 d-flex flex-column gap-3">
            <div class="game-stat-panel" style="border-color:rgba(0,210,255,0.2);">
                <div class="stat-row"><span class="stat-lbl">SCORE</span><span class="stat-val" id="tet-score" style="color:#00d2ff">0</span></div>
            ...
            {{-- Next Piece --}}
            <div class="game-controls-panel" style="border-color:rgba(0,210,255,0.2);">
...
.game-stat-panel,.game-controls-panel { background:rgba(15,23,42,0.7); border:1px solid rgba(0,210,255,0.2); border-radius:12px; padding:20px; backdrop-filter:blur(8px); }
...
const PIECES = [
    {shape:[[1,1,1,1]],          color:'#00d2ff', glow:'rgba(0,210,255,0.8)'},   // I
...
function drawCell(c, x, y, alpha=1) {
    if(!c) return;
    c.shadowColor = PIECES.find(p=>p.color===y?.color||false)?.glow || 'rgba(0,210,255,0.5)';
    c.shadowBlur  = 12;
...
function draw() {
    ...
    // Grid lines
    ctx.strokeStyle='rgba(0,210,255,0.04)'; ctx.lineWidth=0.5;
    ...
    for(let r=0;r<ROWS;r++) for(let c=0;c<COLS;c++) {
        if(board[r][c]) {
            const col=board[r][c];
            ctx.shadowColor=PIECES.find(p=>p.color===col)?.glow||'rgba(0,210,255,0.5)';
            ctx.shadowBlur=8;
